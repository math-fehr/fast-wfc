...
struct Tile {
   ...
   */
  static vector<unsigned> generate_rotation_map(const Symmetry& symmetry) noexcept {
    switch(symmetry) {
    case Symmetry::X:
      return {0};
    case Symmetry::I:
   ...
   */
  static vector<unsigned> generate_reflection_map(const Symmetry& symmetry) noexcept {
    switch(symmetry) {
    case Symmetry::X:
      return {0};
    case Symmetry::I:
   ...
   * on the x axis.
   */
  static vector<vector<unsigned>> generate_action_map(const Symmetry& symmetry) noexcept {
    ...
    vector<vector<unsigned>> action_map(8, vector<unsigned>(size));
   ...
   */
  static vector<Array2D<T>> generate_oriented(Array2D<T> data, Symmetry symmetry) noexcept {
   ...
   */
  Tile(vector<Array2D<T>> data, Symmetry symmetry, double weight) noexcept :
...
private:
   ...
   */
  vector<pair<unsigned,unsigned>> id_to_oriented_tile;
   ...
   */
  static pair<vector<pair<unsigned, unsigned>>, vector<vector<unsigned>>>
   ...
   */
  static vector<array<vector<unsigned>, 4>> generate_propagator(
      const vector<tuple<unsigned, unsigned, unsigned, unsigned>>& neighbors,
      vector<Tile<T>> tiles,
      vector<pair<unsigned, unsigned>> id_to_oriented_tile,
      vector<vector<unsigned>> oriented_tile_ids) {
    size_t nb_oriented_tiles = id_to_oriented_tile.size();
    vector<array<vector<bool>, 4>> dense_propagator(nb_oriented_tiles,
    ...
    for(auto neighbor: neighbors) {
      unsigned tile1 = get<0>(neighbor);
      unsigned orientation1 = get<1>(neighbor);
      unsigned tile2 = get<2>(neighbor);
      unsigned orientation2 = get<3>(neighbor);
      vector<vector<unsigned>> action_map1 = Tile<T>::generate_action_map(tiles[tile1].symmetry);
      vector<vector<unsigned>> action_map2 = Tile<T>::generate_action_map(tiles[tile2].symmetry);
    ...
    vector<array<vector<unsigned>, 4>> propagator(nb_oriented_tiles);
  ...
  Array2D<T> id_to_tiling(Array2D<unsigned> ids) {
    ...
    for(unsigned i = 0; i < ids.height; i++) {
      for(unsigned j = 0; j < ids.width; j++) {
        pair<unsigned, unsigned> oriented_tile = id_to_oriented_tile[ids.get(i,j)];
...
public:
   ...
   */
  TilingWFC(const vector<Tile<T>>& tiles,
            const vector<tuple<unsigned, unsigned, unsigned, unsigned>>& neighbors,
